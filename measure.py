...
#    img1 = 16. + (64.738 * img1[..., 0] + 129.057 * img1[..., 1] + 25.064 * img1[..., 2]) / 256.
#    img2 = 16. + (64.738 * img2[..., 0] + 129.057 * img2[..., 1] + 25.064 * img2[..., 2]) / 256.
...
def metrics(im_dir, label_dir):
    ...
    for item in sorted(glob.glob(im_dir)):
        n += 1
        im1 = Image.open(item).convert('RGB') # test
        name = item.split('/')[-1]
        #print(name)
        #name = name.split('_')[0] + '.JPG'              # for SICE
        
        im2 = Image.open(label_dir + name).convert('RGB')  # label
        (h, w) = im2.size
...
if __name__ == '__main__':
    ...
    im_dir = '.../DHURE-main/test_results/LOL/EI/*.png'  #  test results path
    label_dir = '.../DHURE-main/dataset/PairLIE-testing-dataset/LOL-test/reference/'  # label dataset path
    ...
    print("===> Avg.LPIPS: {:.3f} ".format(avg_lpips.item()))
