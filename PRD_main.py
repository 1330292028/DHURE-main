...
parser.add_argument('--data_train', type=str, default='your dataset path')
...
def train():
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        L1, R1, X1 = model(im1)
        L2, R2, X2 = model(im2)   
        loss_PD = PD_loss(im1, X1)
        loss_RB = RB_loss(L1, R1, im1, X1)
        loss_SR_g = SR_g_loss(R1, R2)
        loss_SR_l = SR_l_loss(R1, R2)
        loss =  loss_PD * 500 + loss_RB + loss_SR_g + loss_SR_l
        ...
        if iteration % 100 == 0:
            if not os.path.exists(opt.output_folder_train):
                ...
                os.mkdir(opt.output_folder_train + 'L1/')
            l1 = L1[0].cpu()
            l1_img = transforms.ToPILImage()(l1.squeeze(0))
            l1_img.save(opt.output_folder_train + '/L1/' + file1[0])
...
def checkpoint(epoch):
    model_out_path = opt.save_folder+"PRD_epoch_{}.pth".format(epoch)
